...
def SJF(processes):
    ...
    while i < len(procs) or ready_queue:
        while i < len(procs) and procs[i].arrival_time <= current_time:
            # Push only 2 elements: (priority, object)
...
def SRTF(processes):
    ...
    while i < len(procs) or ready_queue or running:
        ...
        if running:
            # Check if a shorter process arrived to preempt
...
def RoundRobin(processes, quantum):
    ...
    while i < len(procs) or ready_queue:
        ...
        # Gather any processes that arrived WHILE this one was running
        while i < len(procs) and procs[i].arrival_time <= current_time:
            ready_queue.append(procs[i])
            i += 1

        # Put the incomplete process back at the end of the queue
...
def MLQ(processes, queues):
    high = [p for p in processes if p.priority <= 2]
    low = [p for p in processes if p.priority > 2]
    schedule = []
...
def MLFQ(processes, queues):
    ...
    while i < len(procs) or any(queues):
        while i < len(procs) and procs[i].arrival_time <= current_time:
            ...
            schedule.append((p.pid, start, end))
            while i < len(procs) and procs[i].arrival.time <= current_time:
                queues[0].append(procs[i]); i += 1
                if p.remaining_time > 0:
                    if q_index < 2:
